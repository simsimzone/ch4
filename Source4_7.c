...
/*ungets: same as ungetch but it pushes back the whole s string into the input.
-------   The last input character read by getch is placed at the end of s.
-------   And ofcourse no need to reinvent the wheel and know about how getch and ungetch work 
-------   and so no need to know about the input stack, we just use ungetch().
*/
void ungets(char s [])
{
	int i = 0;
	while (s[i++] != '\0'); // to get the length of s
	while (i > 0)
		ungetch(s[--i]);
}