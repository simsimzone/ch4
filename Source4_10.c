...
void main()
{
	...
	{
		while ((type = getop(s)) != EOF && index < MAXLINE)
		{
			switch (type)
			{
			case NUMBER:
...
int isNumber(char s [])
{
	...
	if (!isdigit(c) && c != '.')
		return 0;
	while (isdigit(s[++i] = nextChar = line[++index])); /* read the integer part, or maybe the decimal part. */
	s[i] = '\0';
	if (nextChar == '.')
	{
		s[i] = '.';
		...
		while (isdigit(s[i++] = c = line[++index]));/* read the decimal part. */
		if (c == '.')
		{
			s[i] = '\0';
...
{
	int c, res;
	while ((s[0] = c = line[++index]) == ' ' || c == '\t'); /*skip spaces*/
